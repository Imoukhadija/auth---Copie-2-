🌐 Application Web Assurance – SAHAM / Bahmou Group Fikri

Gestion complète des contrats, clients, paiements et véhicules

📌 Description

Cette application web permet la gestion complète des données d’assurance pour le groupe Saham – Bahmou / Fikri.
Elle centralise la gestion des clients, véhicules, contrats, garanties, catégories, paiements, annonces, rapports, ainsi qu’un module paramètres entièrement administrable.

L’objectif du système est de faciliter le suivi des contrats d’assurance, l’organisation des paiements, la maintenance et les opérations quotidiennes du bureau d'assurance.

⭐ Fonctionnalités principales
📊 Dashboard – Tableau de bord

Statistiques globales : total des contrats, paiements récents, clients actifs

Notifications internes

Accès rapide aux opérations importantes

👥 Gestion des Clients

Ajouter / modifier / supprimer un client

Informations personnelles et coordonnées

Historique des contrats du client

Dossiers et demandes en cours

🚗 Gestion des Véhicules

Ajout de véhicules associés aux clients

Marque, modèle, immatriculation, type

Suivi des documents et dates d’expiration

🛡️ Gestion des Garanties

CRUD complet des garanties d’assurance

Association des garanties aux contrats

Activation / désactivation

📁 Gestion des Contrats

Création d’un contrat d’assurance

Liaison client ↔ véhicule ↔ garantie

Suivi des dates (début, fin, renouvellement)

Statut : actif, expiré, en attente, suspendu

Exportation PDF (si existant)

💰 Paiements

Ajout d’un paiement pour un contrat

Montant / date / type de paiement

Liste complète des paiements

Historique de facturation

Calcul automatique des restes à payer (si inclus)

📂 Rapports

Rapports filtrés par :

Dates

Clients

Véhicules

Type de contrat

Export PDF (si intégré dans ton code)

Rapport financier global

🔧 Maintenance

Suivi des opérations de maintenance technique

Historique des interventions

Notes internes

🏷️ Catégories

Gestion des catégories de garanties, véhicules ou annonces

Ajout / modification / suppression

📢 Annonces

CRUD des annonces informatives

Affichage dans la page "Accueil" ou tableau de bord

Images / description

⚙️ Paramètres (Settings)

Module complet pour gérer :

Informations du bureau d'assurance

Logo, contacts, horaires

Comptes administrateurs

Permissions (si ton projet utilise roles/permissions)

Configuration générale de l’application

🛠️ Technologies utilisées

Laravel (PHP)

Laravel Blade

MySQL

HTML / CSS / JavaScript

Bootstrap ou Tailwind (selon ton projet)

Middleware Auth Laravel


# 🛡️ SAHAM – Application Web d’Assurance

## 🏠 Page d’accueil (Visiteur)
![Home visiteur](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/homesaham.png)

---

# 🛡️ Espace Administrateur

## 🖼️ Dashboard
![Dashboard](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/dashbord.png)

## 🏘️ Exemple Assurance
![Assurance](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/1764781932_assurance-confort-habitation-560x320.jpg)

---

# 📊 Rapports
![Rapports](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/les%20rapports%20showing.png)

---

# 🗂️ Gestion des Catégories

## 📋 Liste des catégories
![Listes catégories](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/listes%20des%20categories.png)

## 👁️ Détail catégorie
![Show catégorie](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/show%20ds%20categories.png)

---

# 👥 Gestion des Clients

## 📋 Liste des clients
![Listes clients](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/listes%20des%20clients.png)

## ✏️ Modifier un client
![Modifier client](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/modifer%20client%20%20.png)

---

# 🛡️ Gestion des Garanties
![Listes garanties](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/listes%20des%20garanties.png)

![Listes garan](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/listes%20garan.png)

---

# 🚗 Gestion des Véhicules
![Listes véhicules](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/listes%20des%20vehicules.png)

---

# 💳 Gestion des Paiements
![Listes paiement](https://github.com/Imoukhadija/auth---Copie-2-/blob/main/public/images/listes%20des%20paiement.PNG)


📬 Contact

Développeuse : Khadija Imou
